...
class Edrone():
    ...
    def __init__(self):
        ...
        self.err_datay = Float32()
        # initial setting of Kp, Kd and ki for [pitch, roll, yaw]. eg: self.Kp[2] corresponds to Kp value in yaw axis
        ...
        self.err=[0.0 ,0.0 ,0.0] # pitch,roll,yaw
        ...
        self.sample_time = 0.09 # in seconds
...
#        self.Kd[1] = pitch.Kd * 0.002 
    ...
    def pid(self):
        ...
        (self.drone_orientation_euler[0], self.drone_orientation_euler[1], self.drone_orientation_euler[2]) = tf.transformations.euler_from_quaternion([self.drone_orientation_quaternion[0], self.drone_orientation_quaternion[2], self.drone_orientation_quaternion[1], self.drone_orientation_quaternion[3]])
       # rospy.loginfo(self.drone_orientation_euler[0]*57.216966)
        ...
        self.err[0] = self.setpoint_euler[0] - (self.drone_orientation_euler[0] * 180)/3.1459
        self.err[1] = self.setpoint_euler[1] - (self.drone_orientation_euler[1] * 180)/3.1459
        self.err[2] = self.setpoint_euler[2] - (self.drone_orientation_euler[2] * 180)/3.1459
      
        # error sum
        self.erri_p += self.err[0] #* (self.l_time - self.now)
        self.erri_r += self.err[1] #* (self.l_time - self.now)
        self.erri_y += self.err[2] #* (self.l_time - self.now)
        # error diff
        self.errd_p = (self.err[0] - self.prev_values[0]) / (self.l_time - self.now)
        self.errd_r = (self.err[1] - self.prev_values[1]) / (self.l_time - self.now)
        self.errd_y = (self.err[2] - self.prev_values[2]) / (self.l_time - self.now)
        # previous error
        self.prev_values[0] = self.err[0]
        self.prev_values[1] = self.err[1]
        self.prev_values[2] = self.err[2]
        # output
        self.out_pitch  = self.Kp[0] * self.err[0] + self.Ki[0] * self.erri_p + self.Kd[0] * self.errd_p
        self.out_roll = self.Kp[1] * self.err[1] + self.Ki[1] * self.erri_r + self.Kd[1] * self.errd_r
        self.out_yaw   = self.Kp[2] * self.err[2] + self.Ki[2] * self.erri_y + self.Kd[2] * self.errd_y

        #rospy.loginfo(self.out_pitch)
        # pwm for each propeller
...
if __name__ == '__main__':
   try:
    e_drone = Edrone()
    r = rospy.Rate(11.11111)  # specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz
